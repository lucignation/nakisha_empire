...
export default function MarketplaceHomeCatalog({ products }: MarketplaceHomeCatalogProps) {
  ...
  const absoluteMaximumPrice = Math.max(...products.map((product) => product.price));
    ...
    <div className="grid gap-6 lg:grid-cols-[16rem_minmax(0,1fr)]">
          ...
          <CardContent className="space-y-6 p-5">
            <div>
              ...
              <label className="relative block">
                <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#8f7767]" />
                <Input
                  className="h-10 border-[#e1d3c1] bg-[#faf6f1] pl-10"
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs text-[#8f7767]">Up to ₦{maximumPrice.toLocaleString()}</span>
              </div>
              <input
                ...
                value={maximumPrice}
              />
              <div className="flex items-center justify-between text-xs text-[#8f7767]">
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                {categories.map((category) => (
                  <label className="flex items-center gap-3 text-sm text-[#6b4f3a]" key={category}>
                    <input
                      checked={selectedCategories.includes(category)}
                      className="h-4 w-4 rounded border-[#d8c8b4] accent-[#b8924a]"
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                {brands.map((brand) => (
                  <label className="flex items-center gap-3 text-sm text-[#6b4f3a]" key={brand}>
                    <input
      ...
      <div className="space-y-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="rounded-[4px] bg-[#f2ebe0] px-4 py-2 text-sm text-[#6b4f3a]">Fast checkout and nationwide delivery</div>
        </div>
        ...
        {visibleProducts.length > 0 && totalPages > 1 ? (
          <div className="flex flex-col gap-3 rounded-[18px] border border-[#eadfce] bg-white p-4 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-sm text-[#8f7767]">
        ...
        {visibleProducts.length === 0 ? (
          <Card className="border-[#eadfce] bg-white">
            <CardContent className="p-8 text-center">
              ...
              <p className="mt-2 text-sm text-[#8f7767]">Reset the filters or try a different search term.</p>
            </CardContent>
          </Card>
        ) : null}
      </div>
